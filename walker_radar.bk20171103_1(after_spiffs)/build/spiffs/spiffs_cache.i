# 1 "/Users/Sentaro/GitHub/happy-project/walker_radar/components/spiffs/./spiffs_cache.c"
...
# 9 "/Users/Sentaro/GitHub/happy-project/walker_radar/components/spiffs/./spiffs_cache.c" 2
...
s32_t spiffs_object_index_consistency_check(
    spiffs *fs);
# 10 "/Users/Sentaro/GitHub/happy-project/walker_radar/components/spiffs/./spiffs_cache.c" 2
...
static spiffs_cache_page *spiffs_cache_page_get(spiffs *fs, spiffs_page_ix pix) {
  ...
  for (i = 0; i < cache->cpage_count; i++) {
    spiffs_cache_page *cp = ((spiffs_cache_page *)(&((cache)->cpages[(i) * (sizeof(spiffs_cache_page) + ((fs)->cfg.log_page_size))])));
...
static s32_t spiffs_cache_page_free(spiffs *fs, int ix, u8_t write_back) {
  s32_t res = 0;
  spiffs_cache *cache = ((spiffs_cache *)((fs)->cache));
  spiffs_cache_page *cp = ((spiffs_cache_page *)(&((cache)->cpages[(ix) * (sizeof(spiffs_cache_page) + ((fs)->cfg.log_page_size))])));
  if (cache->cpage_use_map & (1<<ix)) {
    if (write_back &&
        (cp->flags & (1<<7)) == 0 &&
        (cp->flags & (1<<0))) {
      u8_t *mem = ((u8_t *)(&((cache)->cpages[(ix) * (sizeof(spiffs_cache_page) + ((fs)->cfg.log_page_size))])) + sizeof(spiffs_cache_page));
      res = (fs)->cfg.hal_write_f((( ((fs)->cfg.phys_addr) + (cp->pix) * ((fs)->cfg.log_page_size) )), (((fs)->cfg.log_page_size)), (mem));
    }
...
s32_t spiffs_phys_rd(
  ...
  s32_t res = 0;
  spiffs_cache *cache = ((spiffs_cache *)((fs)->cache));
  spiffs_cache_page *cp = spiffs_cache_page_get(fs, ( ((addr) - ((fs)->cfg.phys_addr)) / ((fs)->cfg.log_page_size) ));
  cache->last_access++;
  if (cp) {
    ...
    cp->last_access = cache->last_access;
    u8_t *mem = ((u8_t *)(&((cache)->cpages[(cp->ix) * (sizeof(spiffs_cache_page) + ((fs)->cfg.log_page_size))])) + sizeof(spiffs_cache_page));
    memcpy(dst, &mem[( ((addr) - ((fs)->cfg.phys_addr)) % ((fs)->cfg.log_page_size) )], len);
  } else {
    if ((op & (3<<0)) == (1<<0)) {
    ...
    res = spiffs_cache_page_remove_oldest(fs, (1<<7), 0);

    cp = spiffs_cache_page_allocate(fs);
    if (cp) {
      cp->flags = (1<<1);
      cp->pix = ( ((addr) - ((fs)->cfg.phys_addr)) / ((fs)->cfg.log_page_size) );

      s32_t res2 = (fs)->cfg.hal_read_f((addr - ( ((addr) - ((fs)->cfg.phys_addr)) % ((fs)->cfg.log_page_size) )), (((fs)->cfg.log_page_size)), (((u8_t *)(&((cache)->cpages[(cp->ix) * (sizeof(spiffs_cache_page) + ((fs)->cfg.log_page_size))])) + sizeof(spiffs_cache_page))))
      ...
      s32_t res2 = (fs)->cfg.hal_read_f((addr), (len), (dst));
      if (res2 != 0) {
...
s32_t spiffs_phys_wr(
    ...
    u8_t *src) {
  (void)fh;
  spiffs_page_ix pix = ( ((addr) - ((fs)->cfg.phys_addr)) / ((fs)->cfg.log_page_size) );
  spiffs_cache *cache = ((spiffs_cache *)((fs)->cache));
  spiffs_cache_page *cp = spiffs_cache_page_get(fs, pix);
  ...
  if (cp && (op & (7<<2)) != (6<<2)) {
    ...
    u8_t *mem = ((u8_t *)(&((cache)->cpages[(cp->ix) * (sizeof(spiffs_cache_page) + ((fs)->cfg.log_page_size))])) + sizeof(spiffs_cache_page));
    memcpy(&mem[( ((addr) - ((fs)->cfg.phys_addr)) % ((fs)->cfg.log_page_size) )], src, len);
...
spiffs_cache_page *spiffs_cache_page_allocate_by_fd(spiffs *fs, spiffs_fd *fd) {
  ...
  spiffs_cache_page_remove_oldest(fs, (1<<7), 0);
...
void spiffs_cache_init(spiffs *fs) {
  ...
  int i;
  int cache_entries =
      (sz - sizeof(spiffs_cache)) / ((sizeof(spiffs_cache_page) + ((fs)->cfg.log_page_size)));
  ...
  spiffs_cache *c = ((spiffs_cache *)((fs)->cache));

  memset(c->cpages, 0, c->cpage_count * (sizeof(spiffs_cache_page) + ((fs)->cfg.log_page_size)));

  c->cpage_use_map &= ~(c->cpage_use_mask);
  for (i = 0; i < cache.cpage_count; i++) {
    ((spiffs_cache_page *)(&((c)->cpages[(i) * (sizeof(spiffs_cache_page) + ((fs)->cfg.log_page_size))])))->ix = i;
  }
}
