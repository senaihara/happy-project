...
# 60 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/secure_boot.c"
    ...
    if (err != 0)
    {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { ets_printf("\033[0;" "31" "m" "E" " (%d) %s: " "SPI erase failed: 0x%x" "\033[0m" "\n", esp_log_timestamp(), TAG, err); };
        return 
...
# 66 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/secure_boot.c"
    ...
    if (!image) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { ets_printf("\033[0;" "31" "m" "E" " (%d) %s: " "bootloader_mmap(0x1000, 0x%x) failed" "\033[0m" "\n", esp_log_timestamp(), TAG, image_len); };
        return 
...
# 73 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/secure_boot.c"
    ...
    if (((esp_log_level_t) 3) >= ESP_LOG_DEBUG) { ets_printf( "D" " (%d) %s: " "write iv+digest to flash" "\033[0m" "\n", esp_log_timestamp(), TAG); };
    err = bootloader_flash_write(0, &digest,
                           sizeof(digest), esp_flash_encryption_enabled());
    if (err != 0) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { ets_printf("\033[0;" "31" "m" "E" " (%d) %s: " "SPI write failed: 0x%x" "\033[0m" "\n", esp_log_timestamp(), TAG, err); };
        return 
...
esp_err_t esp_secure_boot_permanently_enable(void) {
    ...
    {
        if (((esp_log_level_t) 3) >= ESP_LOG_INFO) { ets_printf("\033[0;" "32" "m" "I" " (%d) %s: " "bootloader secure boot is already enabled, continuing.." "\033[0m" "\n", esp_log_timestamp(), TAG); };
        return 0;
    }

    err = esp_image_verify_bootloader(&image_len);
    if (err != 0) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { ets_printf("\033[0;" "31" "m" "E" " (%d) %s: " "bootloader image appears invalid! error %d" "\033[0m" "\n", esp_log_timestamp(), TAG, err); };
        return err;
    }
...
# 125 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/secure_boot.c"
       && ({ ; (*(volatile uint32_t *)((0x3ff5A000 + 0x058))); }) == 0
        && ({ ; (*(volatile uint32_t *)((0x3ff5A000 + 0x05c))); }) == 0
        && ({ ; (*(volatile uint32_t *)((0x3ff5A000 + 0x060))); }) == 0
        && ({ ; (*(volatile uint32_t *)((0x3ff5A000 + 0x064))); }) == 0
        && ({ ; (*(volatile uint32_t *)((0x3ff5A000 + 0x068))); }) == 0
        && ({ ; (*(volatile uint32_t *)((0x3ff5A000 + 0x06c))); }) == 0
        && ({ ; (*(volatile uint32_t *)((0x3ff5A000 + 0x070))); }) == 0
        && ({ ; (*(volatile uint32_t *)((0x3ff5A000 + 0x074))); }) == 0) {
        if (((esp_log_level_t) 3) >= ESP_LOG_INFO) { ets_printf("\033[0;" "32" "m" "I" " (%d) %s: " "Generating new secure boot key..." "\033[0m" "\n", esp_log_timestamp(), TAG); };
        uint32_t buf[8];
        bootloader_fill_random(buf, sizeof(buf));
        for (int i = 0; i < 8; i++) {
            if (((esp_log_level_t) 3) >= ESP_LOG_VERBOSE) { ets_printf( "V" " (%d) %s: " "EFUSE_BLK2_WDATA%d_REG = 0x%08x" "\033[0m" "\n", esp_log_timestamp(), TAG, i, buf[i]); };
            ({ ; (*(volatile uint32_t *)((0x3ff5A000 + 0x0b8) + 4*i)) = (buf[i]); });
        }
        bzero(buf, sizeof(buf));
        burn_efuses();
        if (((esp_log_level_t) 3) >= ESP_LOG_INFO) { ets_printf("\033[0;" "32" "m" "I" " (%d) %s: " "Read & write protecting new key..." "\033[0m" "\n", esp_log_timestamp(), TAG); };
        ({ ; (*(volatile uint32_t *)((0x3ff5A000 + 0x01c))) = ((1<<8) | (1<<17)); });
        burn_efuses();
...
# 146 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/secure_boot.c"
        ...
        if (((esp_log_level_t) 3) >= ESP_LOG_WARN) { ets_printf("\033[0;" "33" "m" "W" " (%d) %s: " "Using pre-loaded secure boot key in EFUSE block 2" "\033[0m" "\n", esp_log_timestamp(), TAG); };
    }

    if (((esp_log_level_t) 3) >= ESP_LOG_INFO) { ets_printf("\033[0;" "32" "m" "I" " (%d) %s: " "Generating secure boot digest..." "\033[0m" "\n", esp_log_timestamp(), TAG); };
    if (
...
# 153 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/secure_boot.c"
        ...
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { ets_printf("\033[0;" "31" "m" "E" " (%d) %s: " "secure boot generation failed" "\033[0m" "\n", esp_log_timestamp(), TAG); };
        return -1;
    }
    if (((esp_log_level_t) 3) >= ESP_LOG_INFO) { ets_printf("\033[0;" "32" "m" "I" " (%d) %s: " "Digest generation complete." "\033[0m" "\n", esp_log_timestamp(), TAG); };


    if (!efuse_key_read_protected) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { ets_printf("\033[0;" "31" "m" "E" " (%d) %s: " "Pre-loaded key is not read protected. Refusing to blow secure boot efuse." "\033[0m" "\n", esp_log_timestamp(), TAG); };
        return 0x103;
    }
    if (!efuse_key_write_protected) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { ets_printf("\033[0;" "31" "m" "E" " (%d) %s: " "Pre-loaded key is not write protected. Refusing to blow secure boot efuse." "\033[0m" "\n", esp_log_timestamp(), TAG); };
        return 0x103;
    }


    if (((esp_log_level_t) 3) >= ESP_LOG_INFO) { ets_printf("\033[0;" "32" "m" "I" " (%d) %s: " "blowing secure boot efuse..." "\033[0m" "\n", esp_log_timestamp(), TAG); };
    if (((esp_log_level_t) 3) >= ESP_LOG_DEBUG) { ets_printf( "D" " (%d) %s: " "before updating, EFUSE_BLK0_RDATA6 %x" "\033[0m" "\n", esp_log_timestamp(), TAG, ({ ; (*(volatile uint32_t *)((0x3ff5A000 + 0x018))); })); };
    ...
    burn_efuses();
    uint32_t after = ({ ; (*(volatile uint32_t *)((0x3ff5A000 + 0x018))); });
    if (((esp_log_level_t) 3) >= ESP_LOG_DEBUG) { ets_printf( "D" " (%d) %s: " "after updating, EFUSE_BLK0_RDATA6 %x" "\033[0m" "\n", esp_log_timestamp(), TAG, after); };
    if (after & ((1UL << (4)))) {
        if (((esp_log_level_t) 3) >= ESP_LOG_INFO) { ets_printf("\033[0;" "32" "m" "I" " (%d) %s: " "secure boot is now enabled for bootloader image" "\033[0m" "\n", esp_log_timestamp(), TAG); };
        return 0;
    } else {



        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { ets_printf("\033[0;" "31" "m" "E" " (%d) %s: " "secure boot not enabled for bootloader image, EFUSE_RD_ABS_DONE_0 is probably write protected!" "\033[0m" "\n", esp_log_timestamp(), TAG); };
