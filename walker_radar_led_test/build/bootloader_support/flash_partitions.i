  ...
  *num_partitions = 0;
  ...
  for(num_parts = 0; num_parts < (0xC00 / sizeof(esp_partition_info_t)); num_parts++) {
    ...
    if (part->magic == 0xFFFF
      ...
      if (((esp_log_level_t) 3) >= ESP_LOG_DEBUG) { esp_log_write(ESP_LOG_DEBUG, TAG, "D" " (%d) %s: " "partition table verified, %d entries" "\033[0m" "\n", esp_log_timestamp(), TAG, num_parts); };
    ...
    if (part->magic != 0x50AA) {
        if (log_errors) {
            if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "partition %d invalid magic number 0x%x" "\033[0m" "\n", esp_log_timestamp(), TAG, num_parts, part->magic); };
        }
        return 0x103;
    }
    ...
    if (pos->offset > chip_size || pos->offset + pos->size > chip_size) {
        if (log_errors) {
            if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "partition %d invalid - offset 0x%x size 0x%x exceeds flash chip size 0x%x" "\033[0m" "\n", esp_log_timestamp(), TAG, num_parts, pos->offset, pos->size, chip_size); }
                                                                  ;
        }
        return 0x104;
    }
  ...
  if (log_errors) {
      if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "partition table has no terminating entry, not valid" "\033[0m" "\n", esp_log_timestamp(), TAG); };
  }
  return 0x103;
}
