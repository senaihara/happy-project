...
# 76 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/esp_image_format.c" 3 4
   _Bool 
# 76 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/esp_image_format.c"
        do_load = 
# 76 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/esp_image_format.c" 3 4
                  0
# 76 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/esp_image_format.c"
...
# 84 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/esp_image_format.c"
    ...
    if (part->size > 0x1000000) {
        err = 0x102;
        do { if (!silent) { if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "partition size 0x%x invalid, larger than 16MB" "\033[0m" "\n", esp_log_timestamp(), TAG, part->size); }; } goto err; } while(0);
    }

    bzero(data, sizeof(esp_image_metadata_t));
    data->start_addr = part->offset;

    if (((esp_log_level_t) 3) >= ESP_LOG_DEBUG) { esp_log_write(ESP_LOG_DEBUG, TAG, "D" " (%d) %s: " "reading image header @ 0x%x" "\033[0m" "\n", esp_log_timestamp(), TAG, data->start_addr); };
...
goto err;
    ...
    if (data->image.segment_count > 16) {
        do { if (!silent) { if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "image at 0x%x segment count %d exceeds max %d" "\033[0m" "\n", esp_log_timestamp(), TAG, data->start_addr, data->image.segment_count, 16); }; } goto err; } while(0)
                                                                                      ;
    }
    ...
    for(int i = 0; i < data->image.segment_count; i++) {
        ...
        if (((esp_log_level_t) 3) >= ESP_LOG_VERBOSE) { esp_log_write(ESP_LOG_VERBOSE, TAG, "V" " (%d) %s: " "loading segment header %d at offset 0x%x" "\033[0m" "\n", esp_log_timestamp(), TAG, i, next_addr); };
    ...
    if (end_addr < data->start_addr) {
        do { if (!silent) { if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "image offset has wrapped" "\033[0m" "\n", esp_log_timestamp(), TAG); }; } goto err; } while(0);
    }

    data->image_len = end_addr - data->start_addr;
    if (((esp_log_level_t) 3) >= ESP_LOG_VERBOSE) { esp_log_write(ESP_LOG_VERBOSE, TAG, "V" " (%d) %s: " "image start 0x%08x end of last section 0x%08x" "\033[0m" "\n", esp_log_timestamp(), TAG, data->start_addr, end_addr); };
    ...
    if (data->image_len > part->size) {
        do { if (!silent) { if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "Image length %d doesn't fit in partition length %d" "\033[0m" "\n", esp_log_timestamp(), TAG, data->image_len, part->size); }; } goto err; } while(0);
    }
...
# 204 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/esp_image_format.c"
    return 0;
                                                                                             ...
                                                                                             silent)
{
    ...
    if (image->magic != 0xE9) {
        if (!silent) {
            if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "image at 0x%x has invalid magic byte" "\033[0m" "\n", esp_log_timestamp(), TAG, src_addr); };
        }
        err = (0x2000 + 2);
    }
    if (!silent) {
        if (image->spi_mode > ESP_IMAGE_SPI_MODE_SLOW_READ) {
            if (((esp_log_level_t) 3) >= ESP_LOG_WARN) { esp_log_write(ESP_LOG_WARN, TAG, "\033[0;" "33" "m" "W" " (%d) %s: " "image at 0x%x has invalid SPI mode %d" "\033[0m" "\n", esp_log_timestamp(), TAG, src_addr, image->spi_mode); };
        }
        if (image->spi_speed > ESP_IMAGE_SPI_SPEED_80M) {
            if (((esp_log_level_t) 3) >= ESP_LOG_WARN) { esp_log_write(ESP_LOG_WARN, TAG, "\033[0;" "33" "m" "W" " (%d) %s: " "image at 0x%x has invalid SPI speed %d" "\033[0m" "\n", esp_log_timestamp(), TAG, src_addr, image->spi_speed); };
        }
        if (image->spi_size > ESP_IMAGE_FLASH_SIZE_MAX) {
            if (((esp_log_level_t) 3) >= ESP_LOG_WARN) { esp_log_write(ESP_LOG_WARN, TAG, "\033[0;" "33" "m" "W" " (%d) %s: " "image at 0x%x has invalid SPI size %d" "\033[0m" "\n", esp_log_timestamp(), TAG, src_addr, image->spi_size); };
        }
    }
    return err;
}
...
# 248 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/esp_image_format.c"
    ...
    if (err != 0) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "bootloader_flash_read failed at 0x%08x" "\033[0m" "\n", esp_log_timestamp(), TAG, flash_addr); };
        return err;
    }
    if (sha_handle != 
...
# 253 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/esp_image_format.c"
    ...
    if (((esp_log_level_t) 3) >= ESP_LOG_VERBOSE) { esp_log_write(ESP_LOG_VERBOSE, TAG, "V" " (%d) %s: " "segment data length 0x%x data starts 0x%x" "\033[0m" "\n", esp_log_timestamp(), TAG, data_len, data_addr); };
    ...
    if (data_len % 4 != 0) {
        do { if (!silent) { if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "unaligned segment length 0x%x" "\033[0m" "\n", esp_log_timestamp(), TAG, data_len); }; } goto err; } while(0);
    }
...
# 272 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/esp_image_format.c"
    ...
    if (!silent) {
        if (((esp_log_level_t) 3) >= ESP_LOG_INFO) { esp_log_write(ESP_LOG_INFO, TAG, "\033[0;" "32" "m" "I" " (%d) %s: " "segment %d: paddr=0x%08x vaddr=0x%08x size=0x%05x (%6d) %s" "\033[0m" "\n", esp_log_timestamp(), TAG, index, data_addr, load_addr, data_len, data_len, (do_load)?"load":(is_mapping)?"map":""); }
    ...
    if (do_load) {
        ...
        if (end_addr < 0x40000000) {
            ...
            if (end_addr > sp - 32768) {
                if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "Segment %d end address 0x%08x too high (bootloader stack 0x%08x liimit 0x%08x)" "\033[0m" "\n", esp_log_timestamp(), TAG, index, end_addr, sp, sp - 32768); }
                                                                       ;
                return (0x2000 + 2);
            }
        }
    ...
    if(!data) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "bootloader_mmap(0x%x, 0x%x) failed" "\033[0m" "\n", esp_log_timestamp(), TAG, data_addr, data_len); }
                                     ;
        return -1;
    }
# 310 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/esp_image_format.c"
    ...
    for (int i = 0; i < data_len; i += 4) {
        ...
        uint32_t w = src[w_i];
        *checksum ^= w;
# 325 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/esp_image_format.c"
                                                                                                                             ...
                                                                                                                             silent)
{
    if ((segment->data_len & 3) != 0
        ...
        if (!silent) {
            if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "invalid segment length 0x%x" "\033[0m" "\n", esp_log_timestamp(), TAG, segment->data_len); };
        }
        return (0x2000 + 2);
    }
...
# 354 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/esp_image_format.c"
    ...
    if (((esp_log_level_t) 3) >= ESP_LOG_VERBOSE) { esp_log_write(ESP_LOG_VERBOSE, TAG, "V" " (%d) %s: " "segment %d map_segment %d segment_data_offs 0x%x load_addr 0x%x" "\033[0m" "\n", esp_log_timestamp(), TAG, index, map_segment, segment_data_offs, load_addr); }
                                                              ;
    if (map_segment
        && ((segment_data_offs % 0x10000) != (load_addr % 0x10000))) {
        if (!silent) {
            if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "Segment %d load address 0x%08x, doesn't match data 0x%08x" "\033[0m" "\n", esp_log_timestamp(), TAG, index, load_addr, segment_data_offs); }
                                                         ;
        }
        return (0x2000 + 2);
    }
...
# 394 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/esp_image_format.c"
    ...
    if (!load_rtc_memory) {
        if (load_addr >= 0x400C0000 && load_addr < 0x400C2000) {
            if (((esp_log_level_t) 3) >= ESP_LOG_DEBUG) { esp_log_write(ESP_LOG_DEBUG, TAG, "D" " (%d) %s: " "Skipping RTC code segment at 0x%08x\n" "\033[0m" "\n", esp_log_timestamp(), TAG, load_addr); };
            return 
...
# 400 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/esp_image_format.c"
        ...
        if (load_addr >= 0x50000000 && load_addr < 0x50002000) {
            if (((esp_log_level_t) 3) >= ESP_LOG_DEBUG) { esp_log_write(ESP_LOG_DEBUG, TAG, "D" " (%d) %s: " "Skipping RTC data segment at 0x%08x\n" "\033[0m" "\n", esp_log_timestamp(), TAG, load_addr); };
            return 
...
# 435 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/esp_image_format.c"
    ...
    if (err != 0 || checksum != calc) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "Checksum failed. Calculated 0x%x read 0x%x" "\033[0m" "\n", esp_log_timestamp(), TAG, checksum, calc); };
        return (0x2000 + 2);
    }
    if (sha_handle != 
...
static esp_err_t verify_secure_boot_signature(bootloader_sha256_handle_t sha_handle, esp_image_metadata_t *data)
{
    ...
    if (err != 0) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "Secure boot signature verification failed" "\033[0m" "\n", esp_log_timestamp(), TAG); };


        if (((esp_log_level_t) 3) >= ESP_LOG_INFO) { esp_log_write(ESP_LOG_INFO, TAG, "\033[0;" "32" "m" "I" " (%d) %s: " "Calculating simple hash to check for corruption..." "\033[0m" "\n", esp_log_timestamp(), TAG); };
...
# 487 "/Users/Sentaro/esp/esp-idf/components/bootloader_support/src/esp_image_format.c"
            ...
            if (verify_simple_hash(sha_handle, data) != 0) {
                if (((esp_log_level_t) 3) >= ESP_LOG_WARN) { esp_log_write(ESP_LOG_WARN, TAG, "\033[0;" "33" "m" "W" " (%d) %s: " "image corrupted on flash" "\033[0m" "\n", esp_log_timestamp(), TAG); };
            } else {
                if (((esp_log_level_t) 3) >= ESP_LOG_WARN) { esp_log_write(ESP_LOG_WARN, TAG, "\033[0;" "33" "m" "W" " (%d) %s: " "image valid, signature bad" "\033[0m" "\n", esp_log_timestamp(), TAG); };
            }
        }
        return (0x2000 + 2);
    }
...
static esp_err_t verify_simple_hash(bootloader_sha256_handle_t sha_handle, esp_image_metadata_t *data)
{
    ...
    if (memcmp(hash, image_hash, 32) != 0) {
        if (((esp_log_level_t) 3) >= ESP_LOG_ERROR) { esp_log_write(ESP_LOG_ERROR, TAG, "\033[0;" "31" "m" "E" " (%d) %s: " "Image hash failed - image is corrupt" "\033[0m" "\n", esp_log_timestamp(), TAG); };
...
static void debug_log_hash(const uint8_t *image_hash, const char *label)
{
        ...
        if (((esp_log_level_t) 3) >= ESP_LOG_DEBUG) { esp_log_write(ESP_LOG_DEBUG, TAG, "D" " (%d) %s: " "%s: %s" "\033[0m" "\n", esp_log_timestamp(), TAG, label, hash_print); };
